...
class Default(WorkerEntrypoint):
    async def fetch(self, request):
        ...
        if request.method == "GET" and path == "/v1/capabilities":
            return self._response({"schema_version": "1", "temporal_schema_version": TEMPORAL_SCHEMA_VERSION, "temporal_contract_hash": TEMPORAL_CONTRACT_SHA256, "service_version": getattr(self.env, "SERVICE_VERSION", "thin-staging"), "worlds": ["real"], "metrics": ["revenue", "operating_margin", "last_price"]})
        try:
